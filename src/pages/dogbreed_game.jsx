...
function ourReducer(draft, action) {
  ...
  switch (action.type) {
    ...
    case "decreaseTime":
      ...
      return
    case "guessAttempt": //this case for right and wrong answers condition 
      ...
      if (action.value === draft.currentQuestion.answer) {
        ...
        draft.strikes++
        if (draft.strikes >= 3) { // if we click 3 times wrog answer time clock will stop and game over screen is display 
          draft.playing = false
        }
      }
      return
  ...
  function generateQuestion() { // logics for generate random questions from api
    if (draft.bigCollection.length <= 12) { // if there is less number of items to fetch so it will increament and generate more items randomly 
      draft.fetchCount++
    }
...
function HeartIcon(props) { //function is for if we heart, so it fills with pnk and if we click on wrong answer more than 3 ties game will stop autmatically 
  return (
    <svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" fill="currentColor" className={props.className} viewBox="0 0 16 16">
...
function App() {
  const timer = useRef(null)//for countdown the timer on screen using this useref method 
  ...
  useEffect(() => {//condition for decrease time after ever secound 
    if (state.playing) {
  ...
  return (
    <div>
        ...
        <>
          <p className="text-center">
            <span className="text-zinc-800 mr-3">
              {/* here we I added extra things like watch to make this game with timer  */}
              <svg xmlns="http://www.w3.org/2000/svg" width="49" height="49" fill="currentColor" className={"inline-block " + (state.playing ? "animate-spin" : "")} viewBox="0 0 16 16">
              {/* //make it dynamic clock to rotate clock wise  */}
              ...
              <span className="font-mono text-5xl relative top-3 ml-4">0:{state.timeRemaining < 10 ? "0" + state.timeRemaining : state.timeRemaining}</span>
            </span>

            {[...Array(3 - state.strikes)].map((item, index) => {
              return <HeartIcon key={index} className="inline text-yellow-600 mx-1" />
            })}
            {[...Array(state.strikes)].map((item, index) => { //if we click on wrong answer hearts will goes ligh yellow till 3  
              return <HeartIcon key={index} className="inline text-yellow-100 mx-1" />
            })}
          </p>
          <h1 className="text-center font-bold pt-5 pb-20 break-all text-4xl md:text-5xl">{state.currentQuestion.breed}</h1>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-8 px-2">
            {state.currentQuestion.photos.map((photo, index) => {
              return <div onClick={() => dispatch({ type: "guessAttempt", value: index })} key={index} className="rounded-lg h-80 lg:h-80 bg-cover bg-center" style={{ backgroundImage: `url(${photo})` }}></div>
            //this method for on click event for right images for right answer 
            })}
          </div>
        </>
      )}
      ...
      {(state.timeRemaining <= 0 || state.strikes >= 3) && state.currentQuestion && ( //final display for score and highest score after click on wrong answer more than 3 times 
        <div className="fixed top-0 left-0 bottom-0 right-0 bg-black/90 text-white flex justify-center items-center text-center">
          <div>
            {state.timeRemaining <= 0 && <p className="text-7xl mb-5 font-bold">Time's Up!</p>}
            {state.strikes >= 3 && <p className="text-7xl mb-5 font-bold">3 Strikes; You're Out!</p>}

            <p>
              Your score:{" "}
              <span className="text-amber-600">
                <svg xmlns="http://www.w3.org/2000/svg" width="34" height="34" fill="currentColor" className="inline-block relative bottom-1 mx-1" viewBox="0 0 16 16">
            ...
            <p className="mb-6">Your all-time high score: {state.highScore}</p>
